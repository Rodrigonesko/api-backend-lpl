...
router.get('/agendamento', auth, async (req, res) => {
    const { agendado, sort, pesquisa, responsavel, limit, page } = req.query
    try {
        const propostas = await propostaEntrevistaService.getPropostasByAgendamento(agendado, sort, pesquisa, responsavel, limit, page)
        res.status(200).json(propostas)
    } catch (error) {
        res.status(500).json({ message: 'Erro ao buscar propostas' })
    }
