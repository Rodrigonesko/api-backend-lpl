...
const ProjetoAj = mongoose.model('ProjetoAj')
...
module.exports = {
    upload: async (req, res) => {
        try {
            ...
            const liminares = await ProjetoAj.find({
                situacao: 'andamento'
            })
            ...
            for (const item of result) {
                const analista = item.NomeAgente
                ...
                if (analista === 'Barbara Cristina Nunes') {
                    const resultado = {
                        analista,
                        idLiminar,
                        mo,
                    ...
                    if (flag === 0) {
                        const insert = await ProjetoAj.create(resultado)
                        quantidade++
                    }
    ...
    show: async (req, res) => {
        try {
            ...
            const liminares = await ProjetoAj.find().sort('dataVigencia')
    ...
    concluir: async (req, res) => {
        try {
            ...
            const result = await ProjetoAj.findByIdAndUpdate({ _id: id }, {
                analista: analista,
                situacao: 'Concluido',
    ...
    change: async (req, res) => {
        try {
            ...
            const result = await ProjetoAj.findByIdAndUpdate({ _id }, {
                analista: analista
            })
